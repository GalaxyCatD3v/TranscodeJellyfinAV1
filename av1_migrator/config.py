...
class OutputConfig:
    ...
    gpu_device: int = 0  # Dedicated NVIDIA GPU card device index
...
class ProcessingConfig:
    ...
    gpu_device_id: int = 0  # Dedicated NVIDIA GPU device index (dedicated card not CPU iGPU)
    ...
    if target_path and target_path.is_file():
        ...
        if "output" in raw_data and isinstance(raw_data["output"], dict):
            ...
            config.output = OutputConfig(
                ...
                gpu_device=int(out_data.get("gpu_device", config.output.gpu_device)),
